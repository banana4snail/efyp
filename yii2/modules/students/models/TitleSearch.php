<?php

namespace app\modules\students\models;

use Yii;
use yii\base\Model;
use yii\data\ActiveDataProvider;
use app\modules\students\models\Title;

/**
 * TitleSearch represents the model behind the search form about `app\modules\students\models\Title`.
 */
class TitleSearch extends Title
{
    /**
     * @inheritdoc
     */
    public function rules()
    {
        return [
            [['titleID', 'category', 'faculty', 'status', 'supervisor', 'coSupervisor', 'moderator'], 'integer'],
            [['title', 'batch', 'departments', 'descriptions', 'equipment', 'course', 'student_fk'], 'safe'],
        ];
    }

    /**
     * @inheritdoc
     */
    public function scenarios()
    {
        // bypass scenarios() implementation in the parent class
        return Model::scenarios();
    }

    /**
     * Creates data provider instance with search query applied
     *
     * @param array $params
     *
     * @return ActiveDataProvider
     */
    public function search($params)
    {
        $query = Title::find();

        // add conditions that should always apply here

        $dataProvider = new ActiveDataProvider([
            'query' => $query,
        ]);

        $this->load($params);

        if (!$this->validate()) {
            // uncomment the following line if you do not want to return any records when validation fails
            // $query->where('0=1');
            return $dataProvider;
        }

        // grid filtering conditions
        $query->andFilterWhere([
            'titleID' => $this->titleID,
            'category' => $this->category,
            'faculty' => $this->faculty,
            'status' => $this->status,
            'supervisor' => $this->supervisor,
            'coSupervisor' => $this->coSupervisor,
            'moderator' => $this->moderator,
        ]);

        $query->andFilterWhere(['like', 'title', $this->title])
            ->andFilterWhere(['like', 'batch', $this->batch])
            ->andFilterWhere(['like', 'departments', $this->departments])
            ->andFilterWhere(['like', 'descriptions', $this->descriptions])
            ->andFilterWhere(['like', 'equipment', $this->equipment])
            ->andFilterWhere(['like', 'course', $this->course])
            ->andFilterWhere(['like', 'student_fk', $this->student_fk]);

        return $dataProvider;
    }
}
